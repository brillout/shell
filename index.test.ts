...
describe('run()', () => {
  it('basic', async () => {
    const res = await run('git ls-files')
    expect(res.stdout).toContain('package.json')
  })
  it('npm script', async () => {
    const res = await run('pnpm run test-script', { tolerateExitCode: true })
    expect(res.stdout).toContain('hello')
    expect(res.exitCode).toBe(42)
  })
  it('error: command failure', async () => {
    const res = await run('git show 123456789', { tolerateExitCode: true, tolerateStderr: true })
  ...
  it('pnpm dlx', async () => {
    const { stdout } = await run('pnpm dlx github:dalisoft/typos-rs-npm#v1.33.1 --help')
